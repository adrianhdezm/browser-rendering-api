...
describe('app e2e', { timeout: 60_000 }, () => {
  ...
  it('returns html and base64 screenshot from /snapshot', async () => {
    ...
    expect(typeof response.body.screenshot).toBe('string');
    expect(response.body.screenshot.length).toBeGreaterThan(0);

    const screenshotBuffer = Buffer.from(response.body.screenshot, 'base64');
